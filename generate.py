...
def get_style(config, model, instance, gpu=None):
    ...
    if lookup_style:
        ...
        if not style_together:
            ...
            collapsed_image = image.permute(1, 2, 0, 3).contiguous().view(
                feats, h, batch_size//a_batch_size, w*a_batch_size).permute(2, 0, 1, 3)
            collapsed_label = spaced_label.permute(1, 0, 2).contiguous().view(
                num_class, batch_size//a_batch_size, spaced_len*a_batch_size).permute(1, 0, 2)
...
def main(resume, saveDir, gpu=None, config=None, addToConfig=None, fromDataset=True, test=False, arguments=None, style_loc=None, alphabets=False):
    print("Main Start")
    ...
    char_set_path = '/home/lettf/handwriting_line_generation/data/IAM_char_set.json'
        ...
        while True:
            ...
            if action == 'done' or action == 'exit' or 'action' == 'quit' or action == 'q':
                ...
                for b in range(images[0].size(0)):
                    for i in range(len(images)):
                        genStep = (
                            (1-images[i][b].permute(1, 2, 0))*127.5).cpu().numpy().astype(np.uint8)
                        path = os.path.join(
            ...
            # interpolate randomly selected styles, "v" is VAE
                ...
                for b in range(images[0].size(0)):
                    for i in range(len(images)):
                        ...
                        cv2.imwrite(path, genStep)
                    torch.save(styles, os.path.join(
            ...
            # Just random (interpolated) styles, with option for random text
                ...
                for i, instance1 in enumerate(get_valid_data_loader):
                    if instance1['author'][0] == author:
                        ...
                        styles.append(model.extract_style(
            ...
            # from given image's style to other image's style.
                ...
                for b in range(images[0].size(0)):
                    for i in range(len(images)):
                        ...
                        genStep = (
                            (1-images[i][b].permute(1, 2, 0))*127.5).cpu().numpy().astype(np.uint8)
                        path = os.path.join(
                            saveDir, f'{"".join(digits)}.png'.format(b, i))
                        # print('wrote: {}'.format(path))
                        cv2.imwrite(path, genStep)

            elif alphabets:  # from given image's style to other image's style. not interpolate
                path1 = arguments['path0']
                ...
                for text in text_gen:
                    images, stylesInter = interpolate(
                        model, [(style1+style2)/2], text, char_to_idx, gpu)
                    pairImages.append(images)

                digits = list(map(str, range(9)))
                random.shuffle(digits)
                folderName = "".join(digits)
                os.mkdir(os.path.join("outputs", folderName))
                for pair, c in zip(pairImages, list(range(97, 123)) + list(range(65, 91))):
                    for b in range(pair[0].size(0)):
                        for i in range(len(pair)):
                            genStep = (
                                (1-pair[i][b].permute(1, 2, 0))*127.5).cpu().numpy().astype(np.uint8)
                            path = os.path.join(
                                saveDir, folderName, f'{chr(c)}.png'.format(b, i))
                            # print('wrote: {}'.format(path))
                            cv2.imwrite(path, genStep[:, :50])

                for file in os.listdir(f"./outputs/{folderName}"):
                    fileName = file.split(".")[0]
                    subprocess.run(
                        ["convert", f"./outputs/{folderName}/{file}", f"./outputs/{folderName}/{fileName}.pnm"])
                    subprocess.run(
                        ["potrace", f"./outputs/{folderName}/{fileName}.pnm", "-s", "-o", f"./src/{fileName}.svg"])

                subprocess.run(["./svgs2ttf", "metadata.json"])
                return
                ...
                for n in range(int(arguments['image_count'])):
                    ...
                    if image.shape[0] != img_height:
                        ...
                        image = cv2.resize(
                            image, (0, 0), fx=percent, fy=percent, interpolation=cv2.INTER_CUBIC)
                    image = image[..., None]
                ...
                style = model.extract_style(torch.stack(
                    [image[:, :, :min_width] for image in images], dim=0), None, 1)
                ...
                images, stylesInter = interpolate(
                    model, style, text_gen, char_to_idx, gpu)
...
def interpolate_horz(model, style, spaced_label):
    ...
    for strechH in np.arange(1, 1.11, 0.01):
        spaced_label = F.interpolate(
            orig_spaced_label, scale_factor=strechH, mode='linear').permute(2, 0, 1)
    ...
    for strech in np.arange(0.9, 1.01, 0.01):
        spaced_label = F.interpolate(
            orig_spaced_label, scale_factor=strech, mode='linear').permute(2, 0, 1)
        gen = model.generator(spaced_label, style)
        results.append(gen)
    return results
...
def correct_pred(pred, label):
    ...
    dtw = torch.FloatTensor(pred_len+1, label_len+1,
                            batch_size).fill_(float('inf'))
    dtw[0, 0] = 0
    ...
    for i in range(1, pred_len+1):
        for j in range(max(1, i-w), min(label_len, i+w)+1):
            ...
            per_batch_min, history[i-1, j-1] = torch.min(
                torch.stack((dtw[i-1, j], dtw[i-1, j-1], dtw[i, j-1])), dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--alphabets', default=False, action='store_const', const=True,
                        help='Generates alphabets')
    ...
    if args.run is not None:
        ...
        for pair in s:
            ...
            arguments[ss[0]] = ss[1]
        print(arguments)
        if("text_gen" not in arguments.keys()):
    ...
    if args.gpu is not None:
        with torch.cuda.device(args.gpu):
            main(args.checkpoint, args.savedir, gpu=args.gpu,  config=args.config,
                 addToConfig=addtoconfig, test=args.test, arguments=arguments, style_loc=args.style_loc, alphabets=args.alphabets)
    else:
        main(args.checkpoint, args.savedir, gpu=args.gpu,  config=args.config,
             addToConfig=addtoconfig, test=args.test, arguments=arguments, style_loc=args.style_loc, alphabets=args.alphabets)
# checkpoint,
